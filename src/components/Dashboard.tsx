...
import StoryPointsComparison from "./StoryPointsComparison";
  ...
  const handleFiltersChange = (newFilters: Filters) => {
    ...
    if (newFilters.assignee) {
      const assignees = Array.isArray(newFilters.assignee) 
        ...
        : [newFilters.assignee];
      filtered = filtered.filter(
        (item: JiraIssue) => assignees.includes(item.assignee)
      );
    }
        ...
        <div className="mx-auto px-3 lg:px-6 py-2 h-full">
          {!isConnected ? (
            ...
            <div className="h-full flex flex-col gap-2 lg:gap-4">
              ...
              <div className="flex-1 flex flex-col lg:grid lg:grid-cols-4 gap-3 min-h-0">
                {/* Filters Panel - Responsive Sidebar */}
                <div className="lg:col-span-1 flex flex-col order-2 lg:order-1">
                  <div className="bg-white/80 backdrop-blur-sm rounded-lg border border-zinc-200/50 shadow-sm h-auto lg:h-full overflow-hidden">
                ...
                {/* Charts Area - Responsive Main Content */}
                <div className="lg:col-span-3 flex flex-col min-h-0 order-1 lg:order-2">
                  <Tabs
                    defaultValue="scatterplot"
                    className="flex flex-col h-full"
                  >
                    <TabsList className="flex-shrink-0 grid w-full grid-cols-3 sm:grid-cols-6 mb-2 bg-gradient-to-r from-zinc-100 to-zinc-50 backdrop-blur-sm h-10 p-1 rounded-xl border border-zinc-200/50">
                      <TabsTrigger
                        value="scatterplot"
                      ...
                      <TabsTrigger
                        value="storypoints"
                        className="hidden sm:flex items-center gap-1 text-xs px-1 sm:px-2 rounded-lg data-[state=active]:bg-gradient-to-r data-[state=active]:from-indigo-500 data-[state=active]:to-indigo-600 data-[state=active]:text-white data-[state=active]:shadow-lg transition-all duration-200 hover:bg-indigo-50 hover:text-indigo-700"
                      >
                    ...
                    <div className="flex-1 bg-white/80 backdrop-blur-sm rounded-lg border border-zinc-200/50 shadow-sm overflow-hidden">
                      ...
                      <TabsContent
                        ...
                        className="h-full m-0 p-3"
                      >
                        <StoryPointsComparison data={filteredData} />
                      </TabsContent>
