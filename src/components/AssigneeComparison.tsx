...
const AssigneeComparison: React.FC<AssigneeComparisonProps> = ({ data }) => {
  const assigneeMetrics = useMemo(() => {
    const metrics = data.reduce(
      (acc, issue) => {
        ...
        if (["Done", "In Progress", "To Do"].includes(issue.status)) {
          acc[assignee].statusBreakdown[
            issue.status as keyof (typeof acc)[typeof assignee]["statusBreakdown"]
          ] += 1;
        } else {
    ...
    const processedMetrics = Object.values(metrics)
      ...
      .map((metric) => ({
        ...metric,
        avgCycleTime:
          metric.cycleTimeCount > 0
            ? Math.round(metric.cycleTimeSum / metric.cycleTimeCount)
            : 0,
        avgStoryPoints:
          metric.totalTickets > 0
            ? Math.round((metric.totalStoryPoints / metric.totalTickets) * 10) /
              10
            : 0,
        completionRate:
          metric.totalTickets > 0
            ? Math.round(
                (metric.statusBreakdown.Done / metric.totalTickets) * 100
              )
            : 0,
      }));
  ...
  const totalStats = useMemo(() => {
    ...
    const stats = assigneeMetrics.reduce(
      (acc, metric) => ({
        ...
        totalCycleTime:
          acc.totalCycleTime + metric.avgCycleTime * metric.cycleTimeCount,
      ...
      return (
        <div className="bg-white p-3 border rounded-lg shadow-lg">
          ...
          <p className="text-purple-600">
            <span className="font-medium">Ciclo Médio:</span>{" "}
            {data.avgCycleTime} dias
          </p>
              ...
              <div>
                ...
                <p className="text-xl font-bold">
                  {totalStats.totalCycleTimeCount > 0
                    ? Math.round(
                        totalStats.totalCycleTime /
                          totalStats.totalCycleTimeCount
                      )
                    : 0}{" "}
                  dias
                </p>
                ...
                {assigneeMetrics.map((metric, index) => (
                  <tr key={metric.name} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-2 text-right">
                      {metric.totalStoryPoints}
                    </td>
                    <td className="p-2 text-right">{metric.avgStoryPoints}</td>
                    <td className="p-2 text-right">
                      {metric.avgCycleTime} dias
                    </td>
                    <td className="p-2 text-right">{metric.completionRate}%</td>
                    <td className="p-2 text-right">
                      {metric.statusBreakdown.Done}
                    </td>
                    <td className="p-2 text-right">
                      {metric.statusBreakdown["In Progress"]}
                    </td>
                    <td className="p-2 text-right">
                      {metric.statusBreakdown["To Do"]}
                    </td>
                  </tr>
                ))}
              </tbody>
